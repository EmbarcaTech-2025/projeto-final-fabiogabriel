...
Node* fallback_humano() {
    ...
    Node** sequence_children = (Node**)malloc(sizeof(Node*) * 2);
    sequence_children[0] = analogico;
    sequence_children[1] = criar_behavior_node("Retorna failure", Retorna_failure);
    Node* sequencia = criar_node(SEQUENCE, "sequencia", tick_sequence, 2, sequence_children, 0);

    Node** human_children = (Node**)malloc(sizeof(Node*) * 2);
    human_children[0] = botao;
    human_children[1] = sequencia;
...
Node* fallback_roboaleatorio() {
    ...
    Node* sapeca = criar_node(PARALLEL, "Paralelo Sapeca", tick_parallel, 2, sapeca_children, 1);
    Node** Sapeca_children = (Node**)malloc(sizeof(Node*) * 2);
    Sapeca_children[0] = criar_behavior_node("Valor_entre_x_e_x", Valor_entre_x_e_x_Node);
    Sapeca_children[1] = sapeca;
    Node* Sapeca = criar_node(SEQUENCE, "Sapeca", tick_sequence, 2, Sapeca_children, 0);
    Node** olhada_lado_children = (Node**)malloc(sizeof(Node*) * 2);
    olhada_lado_children[0] = criar_behavior_node("Valor_foi_y", Valor_foi_y_Node);
    olhada_lado_children[1] = criar_behavior_node("Expressao Lado", Expressao_lado);
    Node* olhada_lado = criar_node(SEQUENCE, "olhada_lado", tick_sequence, 2, olhada_lado_children, 0);
    Node** aleatorio_children = (Node**)malloc(sizeof(Node*) * 2);
    aleatorio_children[0] = Sapeca;
    aleatorio_children[1] = olhada_lado;
    //aleatorio_children[2] = criar_behavior_node("Expressao_Padrao", Expressao_Padrao_Node);
    return criar_node(SELECTOR, "aleatorio", tick_selector, 2, aleatorio_children, 0);
}
...
Node* loop() {
    ...
    Node** fallback_acoes_children = (Node**)malloc(sizeof(Node*) * 4);
    fallback_acoes_children[0] = criar_behavior_node("esta longe do chao?", Esta_na_açao_longe_chao);
    fallback_acoes_children[1] = fallback_humano();
    fallback_acoes_children[2] = fallback_robo();
    fallback_acoes_children[3] = robo_aleatorio;
    Node* fallback_acoes = criar_node(SELECTOR, "fallback_acoes", tick_selector, 4, fallback_acoes_children, 0);
    ...
    Node** loops_children = (Node**)malloc(sizeof(Node*) * 4);
    ...
    loops_children[2] = Longe_chao;
    loops_children[3] = fallback_acoes;
    return criar_node(PARALLEL, "Paralelo principal", tick_parallel, 4, loops_children, 1);
}
